...
class CultivateTest {
    ...
    fun `test cultivate with tile not allowed by Seishi or Growth Cards`() {
        ...
        testPlayer.treeTileLimit = mutableMapOf(
            TileType.WOOD to 1,   // Allowed by Seishi
            TileType.LEAF to 1,   // Allowed by Seishi
    ...
    fun `test general placing rules`() {
        ...
        playerActionService.allowedTiles =mutableMapOf(
        ...
        currentPlayer.supply.addAll(woodTiles)
        currentPlayer.supply.addAll(leafTiles)
        currentPlayer.supply.addAll(flowerTiles)
        currentPlayer.supply.addAll(fruitTiles)
        currentPlayer.supply.add(invalidTile)
        ...
        playerActionService.cultivate(leafTiles[0], 2, -2)
        ...
        playerActionService.cultivate(flowerTiles[0], 3, -3)
        ...
        val exceptionFruitOneLeaf = assertThrows<IllegalArgumentException> {
            playerActionService.cultivate(fruitTiles[1], -1, -2)
        }
        ...
        playerActionService.cultivate(woodTiles[7], 0, -1)
        playerActionService.cultivate(leafTiles[3], 0, -2)
        ...
        playerActionService.cultivate(woodTiles[5], 2, -1)
        playerActionService.cultivate(leafTiles[4], 3, -1)
        ...
        val exceptionFruitNonAdjacentLeaves = assertThrows<IllegalArgumentException> {
            playerActionService.cultivate(fruitTiles[3], 3, -2)
        }
    ...
    fun `test cultivate triggers red and blue goals  for all difficulties`() {
        ...
        val leafTiles = List(25) { BonsaiTile(TileType.LEAF) }
        currentPlayer.supply.addAll(leafTiles)
        ...
        val flowerTiles = List(35) { BonsaiTile(TileType.FLOWER) }
        currentPlayer.supply.addAll(flowerTiles)
